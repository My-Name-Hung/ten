...
function EventDetail() {
    ...
    const fetchEventDetails = async () => {
      try {
        ...
        const eventResponse = await fetch(
          `https://ten-p521.onrender.com/events/${eventId}`
        );
        ...
        const storesResponse = await fetch(
          `https://ten-p521.onrender.com/event-stores/${eventId}`
        );
  ...
  const filteredStores = stores.filter((store) => {
    ...
    const matchStatus =
      statusFilter === "all" ? true : store.status_type === statusFilter;
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <span
                            className={`flex-shrink-0 px-2 py-1 rounded-full text-xs font-medium ${
                              STATUS_MAPPING[store.status_type]?.color ||
                              "bg-yellow-100 text-yellow-800"
                            }`}
                          >
                            {STATUS_MAPPING[store.status_type]?.label ||
                              "Đang chờ duyệt"}
                          </span>
                        </div>
                    ...
                    <div className="hidden md:flex">
                      ...
                      <div className="w-2/3 p-4">
                        <div className="flex justify-between items-start mb-2">
                          <h3
                            className="font-medium text-red-800 line-clamp-1"
                            title={store.name}
                          >
                            {store.name}
                          </h3>
