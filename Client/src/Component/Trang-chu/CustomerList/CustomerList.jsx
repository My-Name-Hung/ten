...
import { FaMedal, FaSearch, FaTimes } from "react-icons/fa";
...
function CustomerList() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://ten-p521.onrender.com/store-info"
        );
  ...
  const filteredStores = Array.isArray(storeInfo)
    ? storeInfo.filter((store) => {
        const searchFields = [
          store.name,
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex-grow max-w-full mx-auto px-2 sm:px-6 lg:px-8 py-6 bg-gray-50">
          ...
          <div className="text-sm text-gray-600 mb-4 px-2">
            Hiển thị {Math.min(indexOfLastStore, filteredStores.length)} /{" "}
          ...
          <div className="bg-white rounded-lg shadow-md mb-6">
            <div
              className="overflow-x-auto scrollbar-always-visible relative"
              style={{
              ...
              <table className="w-full divide-y divide-gray-200 table-auto">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th
                      scope="col"
                      className="sticky left-[50px] z-20 bg-gray-100 px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap border-r border-gray-200"
                      style={{
                        width: "auto",
                        minWidth: "80px",
                        backgroundColor: "#F3F4F6",
                    ...
                    <th
                      scope="col"
                      className="w-auto px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap"
                    >
                      Chủ liên hệ
                    </th>
                    <th
                    ...
                    <th
                      scope="col"
                      className="w-auto min-w-[200px] px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider"
                    >
                      Địa Chỉ
                    </th>
                    <th
                    ...
                    <th
                      scope="col"
                      className="w-auto min-w-[150px] px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider"
                    >
                      Tên NPP
                    </th>
                    <th
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentStores.map((store) => (
                    <tr key={store.store_id} className="hover:bg-gray-50">
                      {/* Sticky Detail cell */}
                      <td
                        className="sticky left-0 z-10 bg-white px-4 py-3 text-sm border-r"
                      ...
                      >
                        <button
                          ...
                          title="Xem chi tiết"
                        >
                          <IoOpenOutline className="w-5 h-5 text-blue-700 group-hover:text-red-600 transition-colors" />
                        </button>
                      </td>
                      {/* Sticky Code cell */}
                      <td
                        className="sticky left-[50px] z-10 bg-white px-4 py-3 text-sm text-gray-900 whitespace-nowrap border-r"
                        style={{ backgroundColor: "#FFFFFF" }}
                      >
                        {store.store_id}
                      </td>
                      {/* Sticky Name cell */}
                      <td
                        className="sticky left-[80px] z-10 bg-white px-4 py-3 text-sm text-gray-900 whitespace-nowrap border-r border-gray-200"
                        style={{ backgroundColor: "#FFFFFF" }}
                      >
                        {store.name}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div
                          className="text-sm text-gray-900 max-w-[200px] overflow-hidden text-ellipsis"
                          title={store.address}
                        >
                          {store.address}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div
                          className="text-sm text-gray-900 max-w-[150px] overflow-hidden text-ellipsis"
                          title={store.npp_name}
                        >
                          {store.npp_name}
                        </div>
                      </td>
